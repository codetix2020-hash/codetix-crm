...
type DistributionClientProps = {
  role: UserRole
  initialLeads: DistributionLead[]
  initialAgents?: DistributionAgent[]
...
export default function DistributionClient({
  ...
  const [leads, setLeads] = useState<DistributionLead[]>(initialLeads)
  const [agents] = useState<DistributionAgent[]>(initialAgents)
  ...
  const handleAssignLeads = async () => {
    const idsToAssign = selectedLeadIds.length
      ? selectedLeadIds
      : leads.slice(0, assignCount).map((lead) => lead.id)
    ...
    try {
      ...
      setSelectedLeadIds([])
      closeAssignModal()
      setLeads((prev) => prev.filter((lead) => !idsToAssign.includes(lead.id)))
    } catch (error) {
  ...
  const handleAutoDistribution = async () => {
    ...
    try {
      ...
      toast.success(
        `Reparto automático completado 🎉 Total: ${payload.total_distributed}${
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="space-y-1">
        ...
        <p className="text-gray-600">Administra y asigna los leads generados a cada comercial.</p>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 bg-white/40 backdrop-blur-md border border-white/20 rounded-2xl p-4 shadow-lg">
        <div className="flex flex-col">
          ...
          <span className="text-lg font-semibold text-gray-800">{leads.length} sin asignar</span>
        </div>
          ...
          {isAdmin && (
            <>
              ...
              <button
                ...
                disabled={autoDistributing || !leads.length}
              >
      ...
      <section className="bg-white/30 backdrop-blur-lg border border-white/20 rounded-2xl shadow-xl overflow-hidden">
        <header className="px-6 py-4 flex items-center justify-between">
          ...
          <span className="text-sm text-gray-500">{leads.length} leads disponibles</span>
        </header>
              ...
              {leads.length ? (
                leads.map((lead) => (
                  <tr key={lead.id} className="hover:bg-white/40 transition-colors">
                    ...
                    <td className="px-6 py-4 font-medium text-gray-800">{lead.business_name ?? lead.name ?? '—'}</td>
...
function StatCard({
  ...
  return (
    <div className="bg-white/40 backdrop-blur-lg border border-white/20 rounded-2xl shadow-lg p-5 flex items-center justify-between">
      ...
      <div className="p-3 rounded-full bg-white/60 text-brand-600 shadow-inner">{icon}</div>
    </div>
  )
}
